...
function countCheatsThatSaveAtLeast(
  ...
  const allCheats: IteratorObject<
    ...
    .flatMap(function* ({ location }) {
      const openNeighbors = orthogonalNeighbors()
        ...
        .filter((loc) => {
          ...
          return forwardDijkstraAllResults.has(locString) &&
            reverseDijkstraAllResults.has(locString);
        });
      ...
      for (const neighbor of openNeighbors) {
        ...
        const timeSaved = noCheatTime - (closestToStartDijkstraNode.cost +
  ...
  return allCheats.reduce((count) => count + 1, 0);
}
