...
// https://github.com/denoland/std/blob/206ed4250b79b2893d72610d08c347eef88bdc54/collections/sliding_windows_test.ts

// Copyright 2018-2025 the Deno authors. MIT license.

import { assertEquals, assertThrows } from "@std/assert";
import {
  slidingWindows as slidingWindows_,
  SlidingWindowsOptions,
} from "./slidingWindows.ts";

/**
 * Small adapter to our iterable version of `slidingWindows` for this test that
 * expects JSR's array-returning version.
 *
 * The rest of this file is copied as-is.
 */
export function slidingWindows<T>(
  ...
  return Array.from(slidingWindows_(iterable, size, options));
}
...
Deno.test({
  ...
  fn() {
    ...
    // Edit: added test for size greater than input length
    slidingWindowsTest([[1, 2, 3, 4, 5], 8, { partial: true }], [
      [1, 2, 3, 4, 5],
      [2, 3, 4, 5],
      [3, 4, 5],
      [4, 5],
...
// Edit: this test is about behavior specific to arrays that can't apply to
// iterables, so disabling it.
// Deno.test({
